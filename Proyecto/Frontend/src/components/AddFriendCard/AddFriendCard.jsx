...
const AddFriendCard = ( { book }) => {
  ...
  const agregarAmigo = (friendEmail) => {
    const user = JSON.parse(localStorage.getItem("data_user"));
    const dataa = {
      email: user.id,
      friendEmail: friendEmail
    }
    Service.agregarAmigo(dataa)
      .then((res) => {
    ...
    <div className="relative flex flex-col mt-6 text-white bg-red-900 shadow-md bg-clip-border rounded-xl max-w-full sm:max-w-96 p-4">
      <div className="p-6 text-center">
        ...
        <div className="flex justify-center mt-4">
          ...
          <button className="mb-2 sm:mb-0 align-middle select-none font-sans font-bold text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 px-6 rounded-lg bg-zinc-900 hover:bg-zinc-600 text-white shadow-md shadow-gray-900/10 hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none" onClick={() => agregarAmigo(book.email)}>
            Agregar Amigo
          </button>
        </div>
      </div>
